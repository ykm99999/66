...
if [ ! -f u-boot.fip ]; then
    echo "u-boot.fip not generated by make, creating manually..."
    if [ -f u-boot.bin ] && [ -f "$IMMORTALWRT_DIR/staging_dir/image/bl31.bin" ]; then
        tools/fiptool create --soc-fw "$IMMORTALWRT_DIR/staging_dir/image/bl31.bin" --nt-fw u-boot.bin u-boot.fip
    else
        echo "❌ Missing u-boot.bin or bl31.bin, cannot create fip"
        exit 1
    fi
fi
