...
echo "✅ fiptool compiled and copied"
...
# 关键修复：传递 BL31 和 FIPTOOL 变量
make CROSS_COMPILE=aarch64-linux-gnu- \
    BL31="$STAGING_DIR_IMAGE/mt7981-nor-ddr4-bl31.bin" \
    FIPTOOL="$PWD/tools/fiptool" \
    -j$(nproc)

# 如果仍然没有生成 u-boot.fip，手动生成
if [ ! -f u-boot.fip ]; then
    echo "⚠️  u-boot.fip not generated by make, trying manual fip creation..."
    if [ -f u-boot.bin ] && [ -f "$STAGING_DIR_IMAGE/mt7981-nor-ddr4-bl31.bin" ]; then
        tools/fiptool create --soc-fw "$STAGING_DIR_IMAGE/mt7981-nor-ddr4-bl31.bin" --nt-fw u-boot.bin u-boot.fip
    fi
fi
