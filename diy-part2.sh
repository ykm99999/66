...
# ... 此处省略完整的 mtk_mem_init.c 内容（与您之前相同）...
# 请将之前的补丁代码完整复制到这里
# 为了简洁，此处用注释代替，您需保留原有完整补丁代码
...
if [ ${PIPESTATUS[0]} -ne 0 ]; then
    echo "❌ Firmware build failed! Last 200 lines of build.log:"
    tail -200 build.log
    exit 1
fi
...
if [ -n "$SPI_IMAGE" ]; then
    ...
    echo "✅ SPI-NOR rescue image copied"
else
