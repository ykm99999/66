...
# ========== 1. 物理修复 ATF 源码 (全链路溯源切除) ==========
echo "=== Executing ATF Absolute Physical Fix ==="
cd $SOURCE_DIR/arm-trusted-firmware

# 物理修补点：直接对冲突的系统公共头文件进行加固，防止其重定义覆盖
sed -i 's/#define FIRMWARE_WELCOME_STR.*/#ifndef FIRMWARE_WELCOME_STR\n\0\n#endif/' include/plat/common/common_def.h

# 物理覆盖：注入您的 SL3000 V2 内存/Flash 对齐配置
...
find bin/targets/ -type f \( -name "*sysupgrade*" -o -name "*.img.gz" \) -exec cp -v {} "$OUTPUT_DIR/firmware/" \;p
